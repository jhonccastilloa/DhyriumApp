...
- Usa `AppFlex` para contenedores estructurales cuyo propósito principal sea organizar contenido mediante dirección, alineación, distribución, `gap`, padding, flex o dimensiones que ya estén expuestas como props. Conserva en `style` únicamente las propiedades visuales o de layout que `AppFlex` no represente.
- Usa `View` para elementos decorativos o geométricos como rails, puntos, separadores, rellenos y overlays; para nodos implicados en medición, referencias nativas, gestos o drag-and-drop; y para internos de primitivas compartidas o elementos repetidos en listas sensibles al rendimiento cuando `AppFlex` no simplifique claramente el código. No reemplaces `View` por `AppFlex` de forma mecánica.
